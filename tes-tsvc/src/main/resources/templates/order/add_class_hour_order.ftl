...
<script type="text/javascript">
    ...
    function buildPackageHtml(strategyName, packageData){
        ...
        html = html.replace(new RegExp("{price}","gm"), packageData.package_price.sale_price + " = " + packageData.package_price.guide_price + " * " + packageData.package_price.discount_percent);
    ...
    function addOrder(param){
        ...
        if(createOrderResult.response != null && createOrderResult.response.error_code == 0){
            ...
            naxtPageUrl = naxtPageUrl.replace("{userId}", studentId);
