...
import com.qingqing.mock.aspect.QingMock;
...
public class QingMockAspect {
    ...
    private static final Logger logger = LoggerFactory.getLogger(com.qingqing.mock.aspect.QingMockAspect.class);
    ...
    public Object around(final ProceedingJoinPoint pjp, QingMock qingMock) throws Throwable {
        ...
        paramMap.put("mockType", qingMock.type());
    ...
    private static Object convertReturnValue(String mockResp, Class<?> returnClass, Type genericReturnType ) throws NoSuchMethodException, IOException, IllegalAccessException, InvocationTargetException {
        ...
        if(List.class.isAssignableFrom(returnClass) || Set.class.isAssignableFrom(returnClass)){//如果return Class 是List  类或其子类
            if(genericReturnType != null) {
                ...
                if(genericReturnType instanceof ParameterizedType){
                    ...
                    List resultList = com.alibaba.fastjson.JSON.parseArray(mockResp, genericClazz);
        ...
        if(Map.class.isAssignableFrom(returnClass)){//如果return Class 是Map  类或其子类
            if(genericReturnType != null) {
                // 如果是泛型参数的类型
                if(genericReturnType instanceof ParameterizedType){
                    ParameterizedType pt = (ParameterizedType) genericReturnType;
                    return com.alibaba.fastjson.JSON.parseObject(mockResp, pt);
                }
            }
        }
    ...
    private static Object convertToProto(String jsonValue, Class<?> protoClass) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException, IOException {
...
//        String s = "{\"4\":\"上海部门\",\"12\":\"就不高诉你\"}";
        String s = "[\"就不高诉你1\", \"就不高诉你2\"]";

        Method method = com.qingqing.mock.aspect.QingMockAspect.class.getMethod("mySet");;
        Type genericReturnType = method.getGenericReturnType();
        Object obj = convertReturnValue(s, method.getReturnType(), genericReturnType);
        System.out.println(Set.class.isAssignableFrom(obj.getClass()));
        System.out.println(((Set)obj).size());
        System.out.println(((Set)obj).iterator().next().getClass());
    }

    public Map<Long, String> my(){
        return null;
    }
